...
class Generator:
    ...
    def __process_all(self, field, type_field, function_name):
        value = field[type_field]
        self.__target_file.write(function_name + "(val")
    ...
    def __get_mandatory_fields(self):
        ...
        self.__target_file.write(str([field['name'] for field in self.__specification['document'] if field['mandatory']]))
        self.__target_file.write("\n")

    def __get_optional_fields(self):
        self.__target_file.write("    optionals = ")
        self.__target_file.write(str([field['name'] for field in self.__specification['document'] if not field['mandatory']]))
        self.__target_file.write("\n")
